...
//http://www.cs.fsu.edu/~cop4610t/lectures/project2/kthreads/kthreads.pdf <idk how up to date but general info
...
/*
stops a udp_service by setting its corresponding task_struct to null?
*/
...
/*
creates a kernel thread given a udp_service, a function and some data
*/
static void create_thread(struct udp_service *k, int (*funct)(void),
                          void *data) {
  size_t len = strlen(k->name);
  char name[len];
  memcpy(name, k->name, len);
  name[len - 1] = '\0';
  k->k_thread = kthread_run((void *)funct, data, name); //creation of kernel thread
  if (k->k_thread >= 0) { //if kernel thread successfully made
...
/*
initializes a udp_service, sets default values for struct fields and mallocs some space in kernel
also initializes udp socket and creates a thread to execute a given function
*/
void init_service(udp_service **k, char *name, unsigned char *myip, int myport,
                  int (*funct)(void), void *data) {
  *k = kmalloc(sizeof(struct udp_service), GFP_KERNEL); //first we kernel malloc space for the struct
...
/*
service is a higher abstraction of a kthread
stops the kthread associated with the udp_service, releases the socket associated with the service and then frees the actual udp_service struct
*/
...
char *get_service_name(udp_service *k) { return k->name; }
...
int prepare_files(enum operations op, unsigned int ntests) {
  if (op != PRINT) {
    ...
    for (size_t i = 0; i < ntests; i++) {
      ...
      n *= 2;
      file_close(file_open(filen, O_CREAT | O_RDWR | O_TRUNC,
                           S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH));
      f[i] = file_open(filen, O_CREAT | O_RDWR | O_APPEND,
                       S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);
      if (!(f[i])) {
