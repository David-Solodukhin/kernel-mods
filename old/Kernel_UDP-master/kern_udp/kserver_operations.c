...
void troughput(message_data *rcv_buf, message_data *rcv_check) {
  ...
  size_t check_size = get_message_size(rcv_check),
         ...
         size_tmval = sizeof(struct timespec), size_avg = sizeof(average);
  ...
  printk("%s Throughput test: this module will count how many packets it "
         "receives\n",
         get_service_name(udp_server));
  ...
  while (1) {
    ...
    bytes_received =
        udp_receive(get_service_sock(udp_server), &hdr, recv_data, recv_size);
    ...
    diff_time = (arrival_time.tv_sec - departure_time.tv_sec) * _1_SEC_TO_NS +
                arrival_time.tv_nsec - departure_time.tv_nsec;
...
void latency(message_data *rcv_buf, message_data *send_buf,
  ...
  while (1) {
    ...
    if (bytes_received == MAX_MESS_SIZE &&
        memcmp(recv_data, check_data, check_size) == 0) {
      reply.msg_name = (struct sockaddr_in *)hdr.msg_name;
      if ((bytes_sent = udp_send(get_service_sock(udp_server), &reply,
                                 send_data, send_size)) != send_size) {
        printk(KERN_ERR "%s Error %d in sending: server not active\n",
               get_service_name(udp_server), bytes_sent);
      }
    }
...
void print(message_data *rcv_buf, message_data *send_buf,
  ...
  struct msghdr reply, hdr;
  construct_header(&hdr, &dest); //construct udp header using sockaddr_in
  construct_header(&reply, NULL);
  ...
  printk("%s Performing a simple test: this module will wait to receive HELLO "
         "from client, replying with OK\n",
         get_service_name(udp_server));
  ...
  while (1) {
    ...
    if (bytes_received == MAX_MESS_SIZE &&
      ...
      printk(KERN_INFO "%s Received %s (size %d) from %pI4:%hu\n",
             get_service_name(udp_server), recv_data, bytes_received,
      ...
      if ((bytes_sent = udp_send(get_service_sock(udp_server), &reply,
        ...
        printk(KERN_INFO "%s Sent %s (size %zu) to %pI4:%hu\n",
               get_service_name(udp_server), send_data, send_size,
               &address->sin_addr, ntohs(address->sin_port));
      }
    }
  }
